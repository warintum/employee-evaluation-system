...
export default function NewEvaluationItemPage() {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      try {
        ...
        console.error('เกิดข้อผิดพลาดในการโหลดข้อมูลแบบฟอร์ม:', err)
        setServerError(err.response?.data?.error || 'ไม่สามารถโหลดข้อมูลแบบฟอร์มได้')
      } finally {
        setLoading(false)
      }
    }
  ...
  // โหลดข้อมูลแบบฟอร์มประเมินเฉพาะที่เลือก
  const fetchTemplateDetails = async (id: string) => {
    try {
      setTemplateLoading(true)
      
      // โหลดข้อมูลแบบฟอร์ม
      const templateResponse = await axios.get(`/api/evaluation-templates/${id}`)
      ...
      // โหลดรายการหัวข้อประเมินที่มีอยู่แล้ว
      const itemsResponse = await axios.get(`/api/evaluation-items?templateId=${id}`)
      setExistingItems(itemsResponse.data)
      
      // กำหนดลำดับให้กับหัวข้อประเมินใหม่
      if (itemsResponse.data.length > 0) {
        // ใช้ลำดับสูงสุดที่มีอยู่ + 1
        const maxOrder = Math.max(...itemsResponse.data.map((item: any) => item.order))
        setFormData({
          ...formData,
          templateId: id,
          order: maxOrder + 1
...
const validateForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('กำลังบันทึกหัวข้อประเมิน:', formData)
      
      const response = await axios.post('/api/evaluation-items', formData)
      
      console.log('บันทึกหัวข้อประเมินสำเร็จ:', response.data)
      
      // นำทางไปยังหน้าแบบฟอร์มประเมิน
      router.push(`/admin/evaluation-templates/${formData.templateId}/edit`)
    } catch (err: any) {
      console.error('เกิดข้อผิดพลาดในการบันทึกหัวข้อประเมิน:', err)
      setServerError(err.response?.data?.error || 'ไม่สามารถบันทึกหัวข้อประเมินได้')
    } finally {
    ...
    <AppLayout>
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">สร้างหัวข้อประเมินใหม่</h1>
          {templateId ? (
            <Link href={`/admin/evaluation-templates/${templateId}/edit`}>
        ...
        <p className="text-gray-600 mt-1">กรอกข้อมูลเพื่อสร้างหัวข้อประเมินใหม่</p>
      </div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg mb-6 flex items-start">
          ...
          <div>
            <p className="text-blue-600">
              กำลังเพิ่มหัวข้อประเมินลงในแบบฟอร์ม <strong>{templateInfo.name}</strong>
            </p>
            <p className="text-blue-600 text-sm mt-1">
              คะแนนเต็มของแบบฟอร์มนี้คือ {templateInfo.maxScore} คะแนน
              {existingItems.length > 0 && ` และมีหัวข้อประเมินอยู่แล้ว ${existingItems.length} หัวข้อ`}
            </p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <select
                ...
                disabled={!!templateId || templateLoading}
                required
              >
            ...
            <div className="mt-8 flex justify-end">
              ...
              <Button type="submit" isLoading={isSubmitting}>
                <FiSave className="mr-2" /> บันทึกหัวข้อประเมิน
              </Button>
            </div>
          </div>
        </form>
